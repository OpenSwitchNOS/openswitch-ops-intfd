...
vtysh_intf_context_clientcallback(void *p_private)
{
   ...
   {
       cur_state = smap_get(&port_row->other_config, PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_KEY_STR);
       if ((NULL != cur_state) && (strcmp(cur_state, PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_VALUE_FALSE) == 0))
       {
          PRINT_INT_HEADER_IN_SHOW_RUN;
