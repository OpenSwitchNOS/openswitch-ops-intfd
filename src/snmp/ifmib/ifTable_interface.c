...
void _ifTable_initialize_interface(ifTable_registration *reg_ptr,
    ...
    /*PDU context name registration, needed for to idetentify the context
      while retrieve the table using context string */
...
int _mfd_ifTable_get_values(netsnmp_mib_handler *handler,
    ...
    Design approch :
    Based on the context name recevied , filterout the OID from MIB directory and
    compared with port linked OID's(is_oid_belongs_context) and acknowledge the same.
    For Ex: Vrf_red linked with interface One and two ,the follwing OID will be shown.
