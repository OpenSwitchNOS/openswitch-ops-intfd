...
intfd_arbiter_get_layer_key(enum ovsrec_interface_layer_e layer)
{
    switch (layer) {
        case INTERFACE_LAYER_HEALTH:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_HEALTH;
        case INTERFACE_LAYER_SECURITY:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_SECURITY;
        case INTERFACE_LAYER_AGGREGATION:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_AGGREGATION;
        default:
            return NULL;
    }
}
...
intfd_arbiter_get_layer_owner_key(enum ovsrec_interface_layer_e layer)
{
    switch (layer) {
        case INTERFACE_LAYER_HEALTH:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_HEALTH_OWNER;
        case INTERFACE_LAYER_SECURITY:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_SECURITY_OWNER;
        case INTERFACE_LAYER_AGGREGATION:
            return INTERFACE_FORWARDING_STATE_MAP_INTERFACE_AGGREGATION_OWNER;
        default:
            return NULL;
    }
}
...
{
    return blocked ?
            INTERFACE_FORWARDING_STATE_STATE_BLOCKED :
            INTERFACE_FORWARDING_STATE_STATE_FORWARDING;
}
                        ...
                        const struct ovsrec_interface *ifrow)
{
    ...
    /* Check if the oper state of the interface is down */
    oper_state = smap_get(&ifrow->hw_intf_config,
                          INTERFACE_HW_INTF_CONFIG_MAP_ENABLE);
    if (!oper_state || !(STR_EQ(oper_state,
                                INTERFACE_HW_INTF_CONFIG_MAP_ENABLE_TRUE))) {
        /* Set the current layer as blocked and remove the owner. */
        if (!layer->blocked) {
            VLOG_DBG("Blocking %d for interface %s because the oper "
                            ...
                            struct smap *forwarding_state)
{
    ...
    if (last_layer) {
        ...
        smap_replace(forwarding_state, INTERFACE_FORWARDING_STATE_MAP_STATE,
                     state_value);
    } else {
        smap_replace(forwarding_state, INTERFACE_FORWARDING_STATE_MAP_STATE,
                     INTERFACE_FORWARDING_STATE_STATE_FORWARDING);
 ...
 */
void
intfd_arbiter_layer_health_register(
 ...
 */
void
intfd_arbiter_layer_security_register(
        struct intfd_arbiter_layer_class **layer_head)
{
