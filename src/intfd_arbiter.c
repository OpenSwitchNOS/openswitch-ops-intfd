        ...
        struct intfd_arbiter_layer_class **layer_head)
{
    ...
    if (aggregation == NULL) {
        VLOG_ERR("Failed to register aggregation layer - Memory allocation failed.");
        return;
    }
    ...
    if (proto == NULL) {
        VLOG_ERR("Failed to register proto class for aggregation layer - Memory allocation failed.");
        return;
    }
