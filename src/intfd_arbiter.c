 ...
 */
const char *
intfd_arbiter_get_proto_name(enum ovsrec_interface_proto_e proto)
{
    switch (proto) {
        case INTERFACE_PROTO_UDLD:
            return INTERFACE_FORWARDING_STATE_PROTOCOL_UDLD;
        case INTERFACE_PROTO_DOT1X:
            return INTERFACE_FORWARDING_STATE_PROTOCOL_DOT1X;
        case INTERFACE_PROTO_LACP:
 ...
 */
const char *
intfd_arbiter_get_layer_key(enum ovsrec_interface_layer_e layer)
{
    switch (layer) {
        case INTERFACE_LAYER_HEALTH:
            return INTERFACE_FORWARDING_STATE_INTERFACE_HEALTH;
        case INTERFACE_LAYER_SECURITY:
            return INTERFACE_FORWARDING_STATE_INTERFACE_SECURITY;
        case INTERFACE_LAYER_AGGREGATION:
            return INTERFACE_FORWARDING_STATE_INTERFACE_AGGREGATION;
        default:
            return NULL;
    }
}
 ...
 */
const char *
intfd_arbiter_get_layer_owner_key(enum ovsrec_interface_layer_e layer)
{
    switch (layer) {
        case INTERFACE_LAYER_HEALTH:
            return INTERFACE_FORWARDING_STATE_INTERFACE_HEALTH_OWNER;
        case INTERFACE_LAYER_SECURITY:
            return INTERFACE_FORWARDING_STATE_INTERFACE_SECURITY_OWNER;
        case INTERFACE_LAYER_AGGREGATION:
            return INTERFACE_FORWARDING_STATE_INTERFACE_AGGREGATION_OWNER;
        default:
            return NULL;
    }
}
                        ...
                        const struct ovsrec_interface *ifrow)
{
    ...
    if (block) {
        ...
        if (proto->layer->blocked) {
            ...
            VLOG_DBG(
                    "Changing status of %d to blocked with owner as %d for interface %s",
                    proto->layer->id, proto->id, ifrow->name);
        ...
        if (proto->layer->blocked) {
            ...
            if (proto->id == proto->layer->owner) {
                VLOG_DBG(
                        "Changing status of %d to forwarding with owner %d cleared for interface %s",
                        proto->layer->id, proto->id, ifrow->name);
                proto->layer->owner = INTERFACE_PROTO_NONE;
                        ...
                        const struct ovsrec_interface *ifrow)
{
    ...
    if (layer->prev && layer->prev->blocked) {
        ...
        VLOG_DBG(
                "Blocking %d for interface %s because forwarding layer %d is blocked",
                layer->id, ifrow->name, layer->prev->id);
        layer->blocked = true;
        layer->owner = INTERFACE_PROTO_NONE;
        return true;
    }
    ...
    if (!admin_state || !(STR_EQ(admin_state,
        ...
        /* Set the current layer as blocked and remove the owner. */
        if (!layer->blocked) {
            VLOG_DBG("Blocking %d for interface %s because the admin state is down",
                      layer->id, ifrow->name);
        }
                            ...
                            struct smap *forwarding_state)
{
    ...
    while (layer != NULL) {
        ...
        /* Get OVSDB key name for setting the forwarding state of the current layer */
        layer_key = intfd_arbiter_get_layer_key(layer->id);
        /* Get OVSDB key name for setting the owner for this layer dictating the forwarding state. */
        ...
        /* Get the value associated with the forwarding state of the current layer */
        state_value = intfd_arbiter_get_state_value(layer->blocked);

        /* Check if the current layer has an owner */
        if (layer->owner != INTERFACE_PROTO_NONE) {
            /* There is an owner. Set the forwarding state and the owner for this layer
             * based on the information cached in the layer data structure. */
            smap_replace(forwarding_state, layer_key, state_value);
            smap_replace(forwarding_state, layer_owner_key, owner_name);
        } else {
            /* There is no owner. Check if the current forwarding state of the layer
             * is blocked. */
            if (layer->blocked) {
                 ...
                 *
                 * Remove the key,value pair for the forwarding state of this layer
                 * and the asserting protocol. */
                smap_remove(forwarding_state, layer_key);
                smap_remove(forwarding_state, layer_owner_key);
            } else {
                /* The forwarding state is open. Set the forwarding state as open
                 * for current layer and remove any asserting protocol. */
                smap_replace(forwarding_state, layer_key, state_value);
                smap_remove(forwarding_state, layer_owner_key);
            }
        }
    ...
    if (last_layer) {
        ...
        smap_replace(forwarding_state, INTERFACE_FORWARDING_STATE, state_value);
    } else {
...
intfd_arbiter_lacp_state(const struct ovsrec_interface *ifrow)
{
    ...
    if (smap_get(&ifrow->hw_bond_config,
        ...
        && STR_EQ(
                smap_get(&ifrow->hw_bond_config, INTERFACE_HW_BOND_CONFIG_MAP_RX_ENABLED),
                "false")
        && STR_EQ(
                smap_get(&ifrow->hw_bond_config, INTERFACE_HW_BOND_CONFIG_MAP_TX_ENABLED),
                "false")) {
        return true;
    } else {
        return false;
 ...
 */
void
intfd_arbiter_layer_health_register(struct intfd_arbiter_layer_class **layer_head)
{
    ...
    health->id = INTERFACE_LAYER_HEALTH;
    health->owner = INTERFACE_PROTO_NONE;
 ...
 */
void
intfd_arbiter_layer_security_register(struct intfd_arbiter_layer_class **layer_head)
{
    ...
    security->id = INTERFACE_LAYER_SECURITY;
    security->owner = INTERFACE_PROTO_NONE;
 ...
 */
void
intfd_arbiter_layer_aggregation_register(struct intfd_arbiter_layer_class **layer_head)
{
    ...
    aggregation->id = INTERFACE_LAYER_AGGREGATION;
    aggregation->owner = INTERFACE_PROTO_NONE;
