                        ...
                        const struct ovsrec_interface *ifrow)
{
    ...
    /* Get the hw_ready_state of the interface */
    hw_ready_state = smap_get(&ifrow->hw_ready_state,
                              "ready");
    if (!hw_ready_state ||
        !(STR_EQ(hw_ready_state,
                 "true"))) {
        /* Set the current layer as blocked and remove the owner. */
        if (!layer->blocked) {
            VLOG_DBG("Blocking %d for interface %s because the h/w ready "
                    "state is blocked", layer->id, ifrow->name);
        }
...
{
    /* TODO: Read from bond_status once the change is merged */
