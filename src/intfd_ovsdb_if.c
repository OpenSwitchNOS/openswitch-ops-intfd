...
/* Function : add_interface_handle_port_config_mods()
 ...
 */
static int
add_interface_handle_port_config_mods(void)
{
    const struct ovsrec_port *port_row = NULL;
    const struct ovsrec_interface *intf_row = NULL;
    int i;
    ...
    VLOG_DBG("add_interface_handle_port_config_mods\n");
    ...
    if ((OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_port_col_admin,
        ...
        OVSREC_PORT_FOR_EACH (port_row, idl) {
            ...
            if (OVSREC_IDL_IS_ROW_MODIFIED(port_row, idl_seqno)) {
                ...
                if (port_row->n_interfaces < port_data->n_interfaces)
                    return rc;
...
remove_interface()
{
    ...
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_port_col_interfaces,
                idl_seqno)) {
        /* Search for port row which has changed interfaces */
        OVSREC_PORT_FOR_EACH (port_row, idl) {
            ...
            if (OVSREC_IDL_IS_ROW_MODIFIED(port_row, idl_seqno)) {
                ...
                if (port_data->n_interfaces <= port_row->n_interfaces)
                    continue;
                ...
                if (!port_row->n_interfaces) {
                    ...
                    intf_row = port_data->interface[0];
                    intf = shash_find_data(&all_interfaces, intf_row->name);
                ...
                for (i = 0; i < port_row->n_interfaces; i++) {
                    found = 0;
                    for(j = 0; j < port_data->n_interfaces; j++) {
                        if((port_data->interface[i] == port_row->interfaces[j])) {
                    ...
                    if(!found) {
                        /* Reset the inetrface admin state */
                        VLOG_DBG("deleting interface from port\n");
                        intf_row = port_data->interface[i];
                        intf = shash_find_data(&all_interfaces, intf_row->name);
                        VLOG_DBG("reset interface %s\n", intf_row->name);
...
//                goto end;
    ...
    /* Add interface and process port config changes */
    if ((rc = add_interface_handle_port_config_mods()) == 0) {
        /* Else remove interfaces from port */
        VLOG_DBG("Since its not an add call remove_interface\n");
        rc = remove_interface();
    }

    VLOG_DBG("3. port_reconfigure\n");

    VLOG_DBG("5. port_reconfigure\n");
end:
