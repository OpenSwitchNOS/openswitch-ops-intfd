...
del_old_port(struct shash_node *sh_node)
{
    ...
    if (sh_node) {
        ...
        for(j = 0; j < port_data->n_interfaces; j++) {
            ...
            /* logical interface details will not be there in interface table since it has been deleted */
            struct ovsrec_interface *intf = get_matching_interface_row(sh_node->name);
            /* skip this for virtual interfaces */
            if(intf){
                /* Making sure not to reset a physical interface associated with another port */
